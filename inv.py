...
if __name__ == "__main__":
    ...
    inv.SetHostVars(inv.AddHost(group, "192.168.122.11"), "hostname", "slb01")
    inv.SetHostVars(inv.AddHost(group, "192.168.122.11"), "priority", 99)
    inv.SetHostVars(inv.AddHost(group, "192.168.122.11"), "interface", "ens3")
    inv.SetHostVars(inv.AddHost(group, "192.168.122.11"), "state", "MASTER")

    inv.SetHostVars(inv.AddHost(group, "192.168.122.12"), "hostname", "slb02")
    inv.SetHostVars(inv.AddHost(group, "192.168.122.12"), "priority", 98)
    inv.SetHostVars(inv.AddHost(group, "192.168.122.12"), "interface", "ens3")
    inv.SetHostVars(inv.AddHost(group, "192.168.122.12"), "state", "BACKUP")
    ...
    inv.SetHostVars(inv.AddHost(group, "192.168.122.21"), "hostname", "etcd01")
    inv.SetHostVars(inv.AddHost(group, "192.168.122.22"), "hostname", "etcd02")
    inv.SetHostVars(inv.AddHost(group, "192.168.122.23"), "hostname", "etcd03")

    # 增加Master节点信息
    group = inv.AddGroup("master")
    inv.SetHostVars(inv.AddHost(group, "192.168.122.31"), "hostname", "master01")
    inv.SetHostVars(inv.AddHost(group, "192.168.122.32"), "hostname", "master02")
    inv.SetHostVars(inv.AddHost(group, "192.168.122.33"), "hostname", "master03")

    # 增加Node节点信息
    group = inv.AddGroup("node")
    inv.SetHostVars(inv.AddHost(group, "192.168.122.41"), "hostname", "node01")
    inv.SetHostVars(inv.AddHost(group, "192.168.122.42"), "hostname", "node02")
    ...
    inv.SetGlobalVars({
        ...
        "VIP": "192.168.122.10", # 配置Kubernetes ApiServer VIP地址，默认值：N/A（必填项）
